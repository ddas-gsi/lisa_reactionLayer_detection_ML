...
# Placeholders for updating charts without recreating elements
main_hist_ph = st.empty()
col1, col2 = st.columns(2)
hist_l0_ph = col1.empty()
hist_l1_ph = col2.empty()
hist_tot_ph = st.empty()
    ...
    if not paused:
        try:
            with open("latest_batch.json", "r") as f:
                ...
                # Update histograms
                fig_main = px.histogram(df, x="Layer", color="Layer", nbins=3, title="Layer Distribution (Updated every second)")
                main_hist_ph.plotly_chart(fig_main, use_container_width=True)

                fig_l0 = px.histogram(df, x="x1", color="Layer", title="dE_L0 Distribution", nbins=30)
                hist_l0_ph.plotly_chart(fig_l0, use_container_width=True)

                fig_l1 = px.histogram(df, x="x2", color="Layer", title="dE_L1 Distribution", nbins=30)
                hist_l1_ph.plotly_chart(fig_l1, use_container_width=True)

                fig_tot = px.histogram(df, x="x3", color="Layer", title="dE_Tot Distribution", nbins=30)
                hist_tot_ph.plotly_chart(fig_tot, use_container_width=True)
